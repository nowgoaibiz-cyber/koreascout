  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-[76px] left-3 z-50 flex md:hidden items-center justify-center w-9 h-9 rounded-lg bg-white border border-[#E8E6E1] shadow-sm"
        aria-label="Toggle navigation"
      >
        <span className="text-[#1A1916] text-lg">{mobileOpen ? '✕' : '☰'}</span>
      </button>
      ...
      {/* Nav panel */}
      <nav
        className={`fixed left-0 top-[72px] h-[calc(100vh-4.5rem)] w-56 z-40 bg-[#F8F7F4] border-r border-[#E8E6E1] flex flex-col transition-transform duration-200
          ...
          {sections.map(({ id, label, icon }) => {
            ...
            return (
              <button
                key={id}
                type="button"
                onClick={() => { handleClick(id); setMobileOpen(false); }}
