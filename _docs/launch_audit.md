# 랜딩페이지 + Pricing + Free 티어 전수조사
---

## SECTION 1: app/page.tsx (랜딩페이지) — Full File
...
## SECTION 2: app/pricing/page.tsx — Full File
...
## SECTION 3: app/sample-report/page.tsx — Full File
...
## SECTION 4: 현재 Free 티어 관련 로직 정리

### 1. Free 유저가 볼 수 있는 주간 로직 (`FREE_DELAY_DAYS`, `isWeekAvailableForFree` 함수 전체)
...
### 2. 유료 주간 접근 로직 (`isPaid`, `subscriptionStartAt` 관련)
...
(`canAccessWeek`는 Free 분기에서 `week.week_id === freeOpenWeekId`, 유료 분기에서 `isLatestWeek || isAfterSub`를 사용한다.)

### 3. Weekly 페이지에서 `sample-report` 링크가 있는가?

`app/weekly/page.tsx` 문자열 검색 기준 **`/sample-report` 링크 없음**.

---

## SECTION 5: 랜딩페이지 분석

1. **`/sample-report` / `/weekly`:** `app/page.tsx` 안에서 **`href="/sample-report"` 1회**(푸터). **`/weekly` 링크는 본 파일에 없음**.

2. **`app/page.tsx`에 있는 CTA:** `Link href="/pricing"`(브리프 카드); `a href="/signup"`(Scout Free); `CheckoutButton`+Lemon URL(Standard/Alpha); `a href="/waitlist"`(Alpha 만석); `a href="#pricing-cards"`; 푸터 `Link` `/pricing`, `/sample-report`, `/contact`.

3. **가격:** 화면 금액은 주로 **`import { PRICING } from "@/src/config/pricing"`**. 페이지 로컬 상수: **`STANDARD_CHECKOUT_URL`, `ALPHA_CHECKOUT_URL`(24–27행), `ALPHA_MAX = 3000`(28행)**. 데모 카드 수치 **`91 / 100`**, **`Gap Index 54`**, **`Margin 3.8x`**(Intelligence Brief JSX).
...
## SECTION 6: Pricing 페이지 분석

1. Standard/Alpha **표시 금액은 `PRICING` 객체** 기반이며, **월가·일환산은 페이지에 숫자 하드코딩 없음**(config). 결제 **`STANDARD_CHECKOUT_URL`, `ALPHA_CHECKOUT_URL`, `ALPHA_MAX_SPOTS`** 는 파일 상단 상수.

2. **Free:** `Scout Free` 카드, `FEATURE_GROUPS`의 Free 열, 「14-day delay」 카피 등 **있음**.

3. **`/sample-report`:** 파일 내 **없음**.
...
| 작업 | 파일 | 수정 범위 |
|------|------|----------|
| A) 샘플 리포 실제 제품 교체 | `data/sampleReportData.ts` | 데이터 교체 또는 DB 연동 시 제거 |
| A) | `app/sample-report/page.tsx` | 서버 fetch·tier/배너 조정 |
| A) | `components/report/*.tsx` | 필요 필드·이미지 대응 |
| B) 랜딩 샘플 링크 | `app/page.tsx` | 추가 CTA 영역 시; 푸터는 이미 `/sample-report` 존재 |
| B) | `components/Hero.tsx` 등 | 히어로에서 샘플 링크 시 |
| C) Free 정책 반영 | `app/weekly/page.tsx` | 지연일·vault·featured 카피 |
| C) | `app/weekly/[weekId]/page.tsx` | 주간 gate |
| C) | `app/weekly/[weekId]/[id]/page.tsx` | 상세 gate·티저 |
| C) | `lib/auth-server.ts` | `maskReportByTier` 등 |
| C) | `supabase/migrations` RLS | DB 접근 정책 동기화 |
| C) | `app/page.tsx`, `app/pricing/page.tsx` | Free 카피 vs 코드 동기화 |
| D) Pricing 업데이트 | `app/pricing/page.tsx` | 카드·표·복지 |
| D) | `src/config/pricing.ts` | 가격 상수 |
